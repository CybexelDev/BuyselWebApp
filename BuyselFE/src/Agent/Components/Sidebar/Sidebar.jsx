...
import { Home, LandPlot, Shield, Database, Settings, LogOut } from 'lucide-react';
...
const navItems = [
  { icon: RiDashboardFill, label: 'Dashboard', id: 'home' },
  { icon:  FaLandmarkDome , label: 'Property Listing', id: 'property listing' },
  { icon:  FaClipboardList , label: 'Plans', id: 'shield' },
  { icon:  FaUserCog  , label: 'Profile', id: 'data' },
  { icon:  SiGooglemessages, label: 'Enquiry', id: 'settings' },
];
      ...
      <nav className="fixed left-2 top-2 backdrop-blur-md hidden bottom-4 w-64 flex-col border-r border-white/10 bg-white rounded-[40px] md:flex z-50 shadow-2xl  mx-2 my-4">
        ...
        <div className="flex-1 px-4 space-y-3 py-4">
          {navItems.map((item) => (
            ...
            >
              <item.icon size={20} className={`shrink-0 transition-transform group-hover:scale-110 ${active === item.id ? 'drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]' : ''}`} />
              <span className="ml-4 text-sm font-semibold tracking-wide uppercase">{item.label}</span>
        ...
        <div className="p-8 mt-auto">
          <button className="flex w-full items-center gap-3 rounded-2xl p-3 text-black/60 font-bold transition-all hover:bg-red-500 hover:text-white group">
            ...
            <span className="text-xs uppercase tracking-tighter">Terminate Session</span>
          </button>
        </div>
      </nav>
